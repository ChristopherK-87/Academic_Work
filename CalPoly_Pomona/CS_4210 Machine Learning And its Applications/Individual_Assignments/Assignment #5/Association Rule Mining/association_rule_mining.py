...
# To do that, create a dictionary (labels) for each transaction, store the corresponding values for each item
# (e.g., {'Bread': 0, 'Milk': 1}) in that transaction, and when is completed, append the dictionary to the list
# encoded_vals below (this is done for each transaction)
# -->add your python code below
encoded_vals = []
for index, row in df.iterrows():
    labels = {'Bread': 0, 'Wine': 0, 'Eggs': 0, 'Meat': 0, 'Cheese': 0, 'Pencil': 0, 'Diaper': 0, 'Bagel': 0}
...
# (the supportCount below). Then, use the gain formula provided right after.
