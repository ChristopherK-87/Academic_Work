...
DESCRIPTION: This program is desined to perfrom the following matrix operations: addition, subtraction, multiplication.
			This will be accomplished by using two float matrices (100x100 max size) which will be filled by the user manually, randomly, or from a given file.
...
void setMatrixManualOrRandom(float[MAX_SIZE][MAX_SIZE], int, int, int);
int  setMatrixFromFile(std::istream&, float[MAX_SIZE][MAX_SIZE], int, int);
void matrixAddition(float[MAX_SIZE][MAX_SIZE], float[MAX_SIZE][MAX_SIZE], int, int, int, int);
void matrixSubtraction(float[MAX_SIZE][MAX_SIZE], float[MAX_SIZE][MAX_SIZE], int, int, int, int);
void matrixMultiplication(float[MAX_SIZE][MAX_SIZE], float[MAX_SIZE][MAX_SIZE], int, int, int, int);
...
int main() {
	...
	float matrix_A[MAX_SIZE][MAX_SIZE] = { 0.0 };
	float matrix_B[MAX_SIZE][MAX_SIZE] = { 0.0 };
	int row_A = 0, row_B = 0;
	...
	do {
		...
		switch (matrixOperationChoice) {
			case 1: {
				...
				matrixAddition(matrix_A, matrix_B, row_A, column_A, row_B, column_B);
				break;
			}
			case 2: {
				// USER WANTS TO PERFORM MATRIX SUBTRACTION
				matrixSubtraction(matrix_A, matrix_B, row_A, column_A, row_B, column_B);
				break;
			}
			case 3: {
				// USER WANTS TO PERFORM MATRIX MULTIPLICATION
				matrixMultiplication(matrix_A, matrix_B, row_A, column_A, row_B, column_B);
				break;
			}
			case -999: {
...
void setMatrixManualOrRandom(float matrix[MAX_SIZE][MAX_SIZE], int random, int row, int column) {
...
int setMatrixFromFile(std::istream &inFile, float matrix[MAX_SIZE][MAX_SIZE], int row, int column) {
...
// FUNCTION TO ADD MATRIX_A TO MATRIX_B AND PRINT THE RESULTS
void matrixAddition(float matrix_A[MAX_SIZE][MAX_SIZE], float matrix_B[MAX_SIZE][MAX_SIZE], int row_A, int column_A, int row_B, int column_B) {
	...
	if ((row_A == row_B) && (column_A == column_B)) {
		...
		for (int r = 0; r < row_A; r++) {
			for (int c = 0; c < column_A; c++) {
				cout << setw(12) << (matrix_A[r][c] + matrix_B[r][c]) << " ";
			}
			cout << endl;
		}
		...
		cout << "=============================================" << endl;
		cout << "    TIME TAKEN = " << duration.count() << " (microseconds)" << endl << endl << endl;
	}
	else {
...
// FUNCTION TO SUBTRACT MATRIX_A TO MATRIX_B AND PRINT THE RESULTS
void matrixSubtraction(float matrix_A[MAX_SIZE][MAX_SIZE], float matrix_B[MAX_SIZE][MAX_SIZE], int row_A, int column_A, int row_B, int column_B) {
...
// FUNCTION TO MULTIPLY MATRIX_A TO MATRIX_B AND PRINT THE RESULTS
void matrixMultiplication(float matrix_A[MAX_SIZE][MAX_SIZE], float matrix_B[MAX_SIZE][MAX_SIZE], int row_A, int column_A, int row_B, int column_B) {
